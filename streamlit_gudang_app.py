...
def group_by_period(df, period="W"):

    # Jika kosong langsung return DF kosong
    if df.empty:
        return pd.DataFrame()

    # --- Normalisasi nama kolom Supabase ---
    # Sesuaikan dengan struktur tabel transaksi
    rename_map = {
        "created_at": "date",
        "qty": "quantity"
    }
    ...
    # Cek apakah kolom-kolom wajib ada
    required_cols = ["date", "name", "unit", "trx_type", "quantity"]
    ...
    # Hapus baris yang gagal konversi tanggal
    ...
    if period == "W":
        ...
        g = (
            df.groupby(["week", "name", "unit", "trx_type"])["quantity"]
              .sum()
              .reset_index()
        )
        return g
    ...
    if period == "M":
        ...
        g = (
            df.groupby(["month", "name", "unit", "trx_type"])["quantity"]
              .sum()
              .reset_index()
        )
        return g
